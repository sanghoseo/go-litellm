...
SELECT k."token", COALESCE(k."models", ARRAY[]::TEXT[]), COALESCE(k."user_id", ''), COALESCE(k."team_id", ''), COALESCE(t."models", ARRAY[]::TEXT[]), COALESCE(k."project_id", ''), COALESCE(p."models", ARRAY[]::TEXT[]), k."expires", COALESCE(k."blocked", false) OR COALESCE(u."blocked", false) OR COALESCE(t."blocked", false) OR COALESCE(p."blocked", false), k."rpm_limit"
...
WHERE k."token" = $1`, tokenHash).Scan(&key.TokenHash, &key.Models, &key.UserID, &key.TeamID, &key.TeamModels, &key.ProjectID, &key.ProjectModels, &key.ExpiresAt, &key.Blocked, &key.RPMLimit)
